...
export function AdminDemarchesPage() {
  ...
  const [filterType, setFilterType] = useState<TypeDemande | 'tous'>('tous')
  ...
  ), [demandes, filterStatut, filterType])
  ...
  return (
    <div style={{ padding: '32px 36px', maxWidth: 1200, fontFamily: 'var(--font-sans)' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <select value={filterType} onChange={e => setFilterType(e.target.value as TypeDemande | 'tous')} style={selectStyle}>
          <option value="tous">Tous les types</option>
          {(Object.entries(TYPE_DEMANDE_LABELS) as [TypeDemande, string][]).map(([k, l]) => (
            <option key={k} value={k}>{l}</option>
          ))}
        </select>
      </div>
          ...
          <table style={{ width: '100%', minWidth: 760, borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--surface-container)', textAlign: 'left' }}>
                {['Référence', 'Type', 'Nom', 'Contact', 'Statut', 'Assigné à', 'Reçue le', ''].map(h => (
              ...
              {filtered.map(d => {
                ...
                return (
                  <tr key={d.id} style={{ borderTop: '1px solid var(--outline-variant)', cursor: 'pointer' }} onClick={() => openDetail(d)}>
                    ...
                    <td style={{ padding: '12px 16px', fontSize: 13, color: 'var(--on-surface)' }}>{TYPE_DEMANDE_LABELS[d.type]}</td>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 20, color: 'var(--primary)', marginBottom: 4 }}>
                  {TYPE_DEMANDE_LABELS[selected.type]}
                </h2>
